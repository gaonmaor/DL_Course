        ...
        with tf.name_scope('val_tower'):
            with tf.variable_scope('net'):
                ...
                norm_val_image_batch = tf.div(tf.subtract(test_image, params.norm),
                                              params.norm)
                net_segs_soft = net.build(norm_val_image_batch, training=False)
    ...
    with tf.Session(config=config) as sess:
        ...
        for image_filename in file_list:
            try:
                ...
                # Sharpen the edge.
                edges1 = filters.sobel(seg_image == 0)
                edges2 = filters.sobel(seg_image == 1)
                seg_image[(edges1 + edges2) > 0] = 2
